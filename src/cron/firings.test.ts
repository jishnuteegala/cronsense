...
describe("step reset across boundaries", () => {
  ...
  it("range step 10-40/25 in minutes", () => {
    ...
    expect(iso(firings)).toEqual(["2026-01-15T11:10Z", "2026-01-15T11:35Z", "2026-01-15T12:10Z"]);
  });
});
...
describe("DOM/DOW OR semantics (provisional, awaiting empirical verification)", () => {
  ...
  it("uses the OR union only when neither day field originates from a wildcard", () => {
    ...
    expect(iso(firings)).toEqual(["2026-01-02T00:00Z", "2026-01-03T00:00Z", "2026-01-04T00:00Z"]);
  });
...
describe("name tokens in ranges and steps (provisional pending GHA-validator arbitration)", () => {
  ...
  it("fires on every second weekday of mon-fri/2 in mixed case", () => {
    const firings = nextFirings(
      ast("0 9 * * mon-FRI/2"),
      new Date(Date.UTC(2026, 0, 18, 12, 0)),
      3,
    );
    expect(iso(firings)).toEqual(["2026-01-19T09:00Z", "2026-01-21T09:00Z", "2026-01-23T09:00Z"]);
  });
  ...
  it("fires on odd months from JAN/2 across a year rollover", () => {
    ...
    expect(iso(firings)).toEqual(["2026-11-01T00:00Z", "2027-01-01T00:00Z", "2027-03-01T00:00Z"]);
  });

  it("fires only in JAN-MAR/2 months", () => {
    const firings = nextFirings(ast("0 0 15 JAN-MAR/2 *"), T0, 3);
    expect(iso(firings)).toEqual(["2026-03-15T00:00Z", "2027-01-15T00:00Z", "2027-03-15T00:00Z"]);
  });
});
